...
use Wx
  qw(:button :cursor :dialog :filedialog :keycode :icon :font :id :listctrl :misc
...
sub new {
			...
			if ($Slic3r::have_threads) {
				Wx::PostEvent(
					...
					Wx::PlThreadEvent->new(
						-1, $PROGRESS_BAR_EVENT,
						shared_clone( [ $percent, $message ] )
					)
				);
			}
			else {
	...
	$self->{preview_notebook} =
	  Wx::Notebook->new( $self, -1, wxDefaultPosition, [ 335, 335 ],
		wxNB_BOTTOM );
	...
	if ($Slic3r::GUI::have_OpenGL) {
		$self->{canvas3D} = Slic3r::GUI::Plater::3D->new(
			$self->{preview_notebook}, $self->{objects},
			$self->{model},            $self->{config}
		);
		$self->{preview_notebook}->AddPage( $self->{canvas3D}, '3D' );
		$self->{canvas3D}->set_on_select_object($on_select_object);
		$self->{canvas3D}->set_on_double_click($on_double_click);
		$self->{canvas3D}->set_on_right_click(
			sub { $on_right_click->( $self->{canvas3D}, @_ ); } );
		$self->{canvas3D}->set_on_instances_moved($on_instances_moved);
		$self->{canvas3D}->on_viewport_changed(
			sub {
				$self->{preview3D}
				  ->canvas->set_viewport_from_scene( $self->{canvas3D} );
			}
		);
	}

	# Initialize 2D preview canvas
	$self->{canvas} = Slic3r::GUI::Plater::2D->new( $self->{preview_notebook},
		wxDefaultSize, $self->{objects}, $self->{model}, $self->{config} );
	$self->{preview_notebook}->AddPage( $self->{canvas}, '2D' );
	$self->{canvas}->on_select_object($on_select_object);
	$self->{canvas}->on_double_click($on_double_click);
	$self->{canvas}
	  ->on_right_click( sub { $on_right_click->( $self->{canvas}, @_ ); } );
	$self->{canvas}->on_instances_moved($on_instances_moved);

	# Initialize 3D toolpaths preview
	if ($Slic3r::GUI::have_OpenGL) {
		$self->{preview3D} =
		  Slic3r::GUI::Plater::3DPreview->new( $self->{preview_notebook},
			$self->{print} );
		$self->{preview3D}->canvas->on_viewport_changed(
			sub {
				$self->{canvas3D}
				  ->set_viewport_from_scene( $self->{preview3D}->canvas );
			}
		);
		$self->{preview_notebook}->AddPage( $self->{preview3D}, 'Preview' );
		$self->{preview3D_page_idx} =
	...
	if ($Slic3r::GUI::have_OpenGL) {
		$self->{toolpaths2D} =
		  Slic3r::GUI::Plater::2DToolpaths->new( $self->{preview_notebook},
			$self->{print} );
	...
	EVT_NOTEBOOK_PAGE_CHANGED(
		...
		$self->{preview_notebook},
		sub {
			if ( $self->{preview_notebook}->GetSelection ==
				$self->{preview3D_page_idx} )
			{
	...
	if ( !&Wx::wxMSW ) {
		Wx::ToolTip::Enable(1);
		$self->{htoolbar} =
		  Wx::ToolBar->new( $self, -1, wxDefaultPosition, wxDefaultSize,
			wxTB_HORIZONTAL | wxTB_TEXT | wxBORDER_SIMPLE | wxTAB_TRAVERSAL );
		$self->{htoolbar}->AddTool(
			TB_ADD,
			"Add…",
			Wx::Bitmap->new( "$Slic3r::var/brick_add.png", wxBITMAP_TYPE_PNG ),
			''
		);
		$self->{htoolbar}->AddTool(
			TB_REMOVE,
		...
		$self->{htoolbar}->AddTool( TB_RESET, "Delete All",
			Wx::Bitmap->new( "$Slic3r::var/cross.png", wxBITMAP_TYPE_PNG ),
			'' );
		$self->{htoolbar}->AddTool( TB_ARRANGE, "Arrange",
			Wx::Bitmap->new( "$Slic3r::var/bricks.png", wxBITMAP_TYPE_PNG ),
			'' );
		$self->{htoolbar}->AddSeparator;
		$self->{htoolbar}->AddTool( TB_MORE, "More",
			Wx::Bitmap->new( "$Slic3r::var/add.png", wxBITMAP_TYPE_PNG ), '' );
		$self->{htoolbar}->AddTool( TB_FEWER, "Fewer",
			Wx::Bitmap->new( "$Slic3r::var/delete.png", wxBITMAP_TYPE_PNG ),
			'' );
		$self->{htoolbar}->AddSeparator;
		$self->{htoolbar}->AddTool(
			TB_45CCW,
		...
		$self->{htoolbar}->AddTool(
			TB_SCALE,
			"Scale…",
			Wx::Bitmap->new( "$Slic3r::var/arrow_out.png", wxBITMAP_TYPE_PNG ),
			''
		);
		$self->{htoolbar}->AddTool(
			TB_SPLIT, "Split",
		...
		$self->{htoolbar}->AddTool( TB_CUT, "Cut…",
			Wx::Bitmap->new( "$Slic3r::var/package.png", wxBITMAP_TYPE_PNG ),
			'' );
		$self->{htoolbar}->AddSeparator;
		$self->{htoolbar}->AddTool( TB_SETTINGS, "Settings…",
			Wx::Bitmap->new( "$Slic3r::var/cog.png", wxBITMAP_TYPE_PNG ), '' );
	}
	else {
		my %tbar_buttons = (
			add         => "Add…",
		...
		for (
			qw(add remove reset arrange increase decrease rotate45ccw rotate45cw changescale split cut settings)
		  )
		{
			$self->{"btn_$_"} =
			  Wx::Button->new( $self, -1, $tbar_buttons{$_}, wxDefaultPosition,
	...
	$self->{list} = Wx::ListView->new(
		...
		wxDefaultPosition,
		wxDefaultSize,
		wxLC_SINGLE_SEL | wxLC_REPORT | wxBORDER_SUNKEN | wxTAB_TRAVERSAL |
		  wxWANTS_CHARS
	);
	$self->{list}->InsertColumn( 0, "Name",   wxLIST_FORMAT_LEFT,   145 );
	$self->{list}->InsertColumn( 1, "Copies", wxLIST_FORMAT_CENTER, 45 );
	$self->{list}->InsertColumn( 2, "Scale", wxLIST_FORMAT_CENTER,
	...
	EVT_KEY_DOWN(
		...
		sub {
			...
			if ( $event->GetKeyCode == WXK_TAB ) {
				$list->Navigate( $event->ShiftDown
					? &Wx::wxNavigateBackward
					: &Wx::wxNavigateForward );
			}
			else {
				$event->Skip;
			}
	...
	$self->{btn_export_gcode} =
	  Wx::Button->new( $self, -1, "Export G-code…",
		wxDefaultPosition, [ -1, 30 ], wxBU_LEFT );
	$self->{btn_send_gcode} = Wx::Button->new( $self, -1, "Send to printer",
		wxDefaultPosition, [ -1, 30 ], wxBU_LEFT );
	$self->{btn_export_stl} = Wx::Button->new( $self, -1, "Export STL…",
		wxDefaultPosition, [ -1, 30 ], wxBU_LEFT );
	$self->{btn_export_svg} =
	  Wx::Button->new( $self, -1, "Slice to SV&G…",
		wxDefaultPosition, [ -1, 30 ], wxBU_LEFT );
	...
	if ($Slic3r::GUI::have_button_icons) {
		...
		for ( grep $self->{"btn_$_"}, keys %icons ) {
			$self->{"btn_$_"}->SetBitmap(
				Wx::Bitmap->new( "$Slic3r::var/$icons{$_}", wxBITMAP_TYPE_PNG )
			);
		}
	}
	...
	EVT_BUTTON(
		...
		sub {
			$self->{send_gcode_file} =
			  $self->export_gcode( Wx::StandardPaths::Get->GetTempDir() );
			Slic3r::thread_cleanup();
		}
	);
	...
	if ( $self->{htoolbar} ) {
		...
		EVT_TOOL( $self, TB_REMOVE, sub { $self->remove() } )
		  ;    # explicitly pass no argument to remove
		...
		EVT_BUTTON( $self, $self->{btn_remove}, sub { $self->remove() } )
		  ;    # explicitly pass no argument to remove
		...
		EVT_BUTTON( $self, $self->{btn_rotate45cw},
			sub { $_[0]->rotate(-45) } );
		EVT_BUTTON( $self, $self->{btn_rotate45ccw},
			sub { $_[0]->rotate(45) } );
		EVT_BUTTON(
			$self,
			$self->{btn_changescale},
			sub { $self->changescale(undef); }
		);
		EVT_BUTTON( $self, $self->{btn_split}, sub { $self->split_object; } );
		EVT_BUTTON( $self, $self->{btn_cut}, sub { $_[0]->object_cut_dialog } );
		EVT_BUTTON( $self, $self->{btn_settings},
	...
	EVT_COMMAND(
		$self, -1,
		$THUMBNAIL_DONE_EVENT,
		sub {
			my ( $self, $event ) = @_;
			my ($obj_idx) = @{ $event->GetData };
			return
			  if !$self->{objects}[$obj_idx]
			  ;    # object was deleted before thumbnail generation completed
...

	{
		...
		if ( $self->GetFrame->{mode} eq 'expert' ) {
			$presets = $self->{presets_sizer} =
			  Wx::FlexGridSizer->new( 3, 2, 1, 2 );
			...
			for my $group (qw(print filament printer)) {
				my $text =
				  Wx::StaticText->new( $self, -1, "$group_labels{$group}:",
					wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT );
				$text->SetFont($Slic3r::GUI::small_font);
				my $choice =
				  Wx::BitmapComboBox->new( $self, -1, "", wxDefaultPosition,
				...
				$presets->Add( $choice, 1,
					wxALIGN_CENTER_VERTICAL | wxEXPAND | wxBOTTOM, 0 );
			}
		}
		...
		my $object_info_sizer;
		{
			...
			while ( my $field = shift @info ) {
				my $label = shift @info;
				my $text =
				  Wx::StaticText->new( $self, -1, "$label:", wxDefaultPosition,
					wxDefaultSize, wxALIGN_LEFT );
				$text->SetFont($Slic3r::GUI::small_font);
				$grid_sizer->Add( $text, 0 );

				$self->{"object_info_$field"} =
				  Wx::StaticText->new( $self, -1, "", wxDefaultPosition,
					wxDefaultSize, wxALIGN_LEFT );
				$self->{"object_info_$field"}
				  ->SetFont($Slic3r::GUI::small_font);
				if ( $field eq 'manifold' ) {
					$self->{object_info_manifold_warning_icon} =
					  Wx::StaticBitmap->new(
						$self, -1,
						Wx::Bitmap->new(
							"$Slic3r::var/error.png", wxBITMAP_TYPE_PNG
						)
					  );
					$self->{object_info_manifold_warning_icon}->Hide;

					my $h_sizer = Wx::BoxSizer->new(wxHORIZONTAL);
					$h_sizer->Add( $self->{object_info_manifold_warning_icon},
						0 );
		...
		$right_sizer->Add( $presets, 0, wxEXPAND | wxTOP, 10 )
		  if defined $presets;
...
sub _on_select_preset {
	...
	if ( $group eq 'filament' && @{ $self->{preset_choosers}{filament} } > 1 ) {
		...
		$Slic3r::GUI::Settings->{presets}{filament} =
		  $choice->GetString( $filament_presets[0] ) . ".ini";
		$Slic3r::GUI::Settings->{presets}{"filament_${_}"} =
		  $choice->GetString( $filament_presets[$_] )
	...
	foreach my $choice (@choosers) {
		...
		foreach my $preset (@$presets) {
			...
			if ( $group eq 'filament' ) {
				...
				if ( $preset->default ) {
					$bitmap =
					  Wx::Bitmap->new( "$Slic3r::var/spool.png",
						wxBITMAP_TYPE_PNG );
				}
				else {
					$rgb_hex =~ s/^#//;
				...
				$bitmap = Wx::Bitmap->new( "$Slic3r::var/printer_empty.png",
					wxBITMAP_TYPE_PNG );
			}
		...
		if ( $selected <= $#$presets ) {
			if ($is_dirty) {
				$choice->SetString( $selected,
					$choice->GetString($selected) . " (modified)" );
			}
...
sub filament_presets {
	...
	return map scalar( $_->GetSelection ),
	  @{ $self->{preset_choosers}{filament} };
}
...
sub load_file {
	...
	my $process_dialog =
	  Wx::ProgressDialog->new( 'Loading…', "Processing input file…",
		100, $self, 0 );
...
sub load_model_objects {
	...
	my $bed_shape =
	  Slic3r::Polygon->new_scale( @{ $self->{config}->bed_shape } );
...
# if object has no defined position(s) we need to rearrange everything after loading
...

		{
		 # if the object is too large (more than 5 times the bed), scale it down
			...
			my $ratio =
			  max( @$size[ X, Y ] ) / unscale( max( @$bed_size[ X, Y ] ) );
			if ( $ratio > 5 ) {
...
'Your object appears to be too large, so it was automatically scaled down to fit your print bed.',
	...
	foreach my $obj_idx (@obj_idx) {
		...
		$self->{list}->SetItemFont( $obj_idx,
			Wx::Font->new( 10, wxDEFAULT, wxNORMAL, wxNORMAL ) )
		  if $self->{list}->can('SetItemFont')
		  ;    # legacy code for wxPerl < 0.9918 not supporting SetItemFont()

		$self->{list}->SetItem( $obj_idx, 1, $model_object->instances_count );
		$self->{list}->SetItem( $obj_idx, 2,
			( $model_object->instances->[0]->scaling_factor * 100 ) . "%" );
...
sub bed_centerf {
	...
	return Slic3r::Pointf->new( unscale( $bed_center->x ),
		unscale( $bed_center->y ) );    #)
}
...
sub reset {
	...
	$self->update;
	@Fls = ();
	Slic3r::Print::count_initialisation;
}
...
sub increase {
	...
	for my $i ( 1 .. $copies ) {
		$instance = $model_object->add_instance(
			offset =>
			  Slic3r::Pointf->new( map 10 + $_, @{ $instance->offset } ),
...
sub set_number_of_copies {
	...
	# prompt user
	my $copies = Wx::GetNumberFromUser(
		"", "Enter the number of copies of the selected object:",
		"Copies", $model_object->instances_count,
		0, 1000, $self
	);
...
sub setPosition {
	...
	if ( !defined $position ) {
		...
		$position = Wx::GetNumberFromUser(
			"",
			"Enter the rotation angle:",
			"Rotate around $axis_name axis",
			$model_instance->rotation, -10000, 10000, $self
		);
		return if !$position || $position == -1;
		$position =
		  0 - $position;    # rotate clockwise (be consistent with button icon)^
...
sub rotate {
	...
	if ( !defined $angle ) {
		my $axis_name = $axis == X ? 'X' : $axis == Y ? 'Y' : 'Z';
		$angle = Wx::GetNumberFromUser(
			"",
			"Enter the rotation angle:",
			"Rotate around $axis_name axis",
			$model_instance->rotation, -364, 364, $self
		);
...
sub changescale {
	...
	if ( defined $axis ) {
		my $axis_name = $axis == X ? 'X' : $axis == Y ? 'Y' : 'Z';
		my $scale = Wx::GetNumberFromUser(
			"",
			"Enter the scale % for the selected object:",
...
# max scale factor should be above 2540 to allow importing files exported in inches
		my $scale = Wx::GetNumberFromUser(
			"", "Enter the scale % for the selected object:",
			'Scale', $model_instance->scaling_factor * 100,
			0, 100000, $self
		);
		return if !$scale || $scale < 0;
...
sub arrange {
	...
	my $bb = Slic3r::Geometry::BoundingBoxf->new_from_points(
		$self->{config}->bed_shape );
	eval {
		$self->{model}
		  ->arrange_objects( $self->GetFrame->config->min_object_distance,
			$bb );
	};
...
# into the same model object, thus causing duplicates when we call load_model_objects()
	my $new_model =
	  $self->{model}->clone;    # store this before calling get_object()
	...
	if ( $current_model_object->volumes_count > 1 ) {
		Slic3r::GUI::warning_catcher($self)
		  ->(
"The selected object can't be split because it contains more than one volume/material."
		  );
		return;
	}
...
sub start_background_process {
	...
	# apply extra variables
	{
		...
		$self->{print}->placeholder_parser->set( $_, $extra->{$_} )
		  for keys %$extra;
	}

	# start thread
	@_ = ();
	$self->{process_thread} = Slic3r::spawn_thread(
		sub {
			eval { $self->{print}->process; };
			if ($@) {
				Slic3r::debugf "Discarding background process error: $@\n";
				Wx::PostEvent( $self,
					Wx::PlThreadEvent->new( -1, $PROCESS_COMPLETED_EVENT, $@ )
				);
			}
			else {
				Wx::PostEvent(
					$self,
					Wx::PlThreadEvent->new(
						-1, $PROCESS_COMPLETED_EVENT, undef
					)
				);
			}
			Slic3r::thread_cleanup();
...
sub pause_background_process {
	...
	elsif ( defined $self->{apply_config_timer}
		&& $self->{apply_config_timer}->IsRunning )
	{
...
sub export_gcode {
	...
	# select output file
	if ($output_file) {
		$self->{export_gcode_output_file} =
		  $self->{print}->expanded_output_filepath($output_file);
	}
	else {
		my $default_output_file =
		  $self->{print}->expanded_output_filepath( $main::opt{output} );
		my $dlg = Wx::FileDialog->new(
			$self,
			'Save G-code file as:',
			wxTheApp->output_path( dirname($default_output_file) ),
			basename($default_output_file),
			&Slic3r::GUI::FILE_WILDCARDS->{gcode},
	...
	if ($Slic3r::have_threads) {
		$self->statusbar->SetCancelCallback(
			sub {
				$self->stop_background_process;
		...
		eval {
			$self->{print}->process;
			$self->{print}->export_gcode(
				output_file => $self->{export_gcode_output_file} );
		};
...
sub on_process_completed {
	...
	if ( $self->{export_gcode_output_file} ) {
		...
		$self->{export_thread} = Slic3r::spawn_thread(
			sub {
				eval {
					$_thread_self->{print}->export_gcode( output_file =>
						  $_thread_self->{export_gcode_output_file} );
				};
				if ($@) {
					Wx::PostEvent(
						$_thread_self,
						Wx::PlThreadEvent->new(
							-1, $ERROR_EVENT, shared_clone( [$@] )
						)
					);
					Wx::PostEvent(
						$_thread_self,
						Wx::PlThreadEvent->new(
							-1, $EXPORT_COMPLETED_EVENT, 0
						)
					);
				}
				else {
...
sub on_export_completed {
	...
	if ($result) {
		if ( $self->{send_gcode_file} ) {
			...
			$message =
			  "G-code file exported to " . $self->{export_gcode_output_file};
		}
	}
	else {
...
# Also, since we need to read from filesystem we process it through encode_path()
	...
	if ( $res->is_success ) {
		...
		my $message =
		  "Error while uploading to the OctoPrint server: " . $res->status_line;
...
sub export_svg_ {
	...
	my $input_file;
	my $dir =
	     $Slic3r::GUI::Settings->{recent}{skein_directory}
	...
	foreach my $choice (@Fls) {
		...
		{
			my $bed_shape =
			  Slic3r::Polygon->new_scale( @{ $config->bed_shape } );
			$print_center = Slic3r::Pointf->new_unscale(
				@{ $bed_shape->bounding_box->center } );
		}
...

		{
			...
			$sprint->placeholder_parser->set( $_, $extra->{$_} )
			  for keys %$extra;
		}
		...
		if ( $params{reslice} ) {
			...
			$output_file = Slic3r::decode_path($output_file);
			$qs_last_output_file = $output_file unless $params{export_svg};
			$Slic3r::GUI::Settings->{_}{last_output_path} =
			  dirname($output_file);
			wxTheApp->save_settings;
		}
...
# this method gets executed in a separate thread by wxWidgets since it's a button handler
	...
	Wx::MessageDialog->new( $self, "end  slicing  !",
		'Error', wxOK | wxICON_ERROR )->ShowModal;
...
sub export_stl {
	...
	Slic3r::Format::STL->write_file( $output_file, $self->{model},
		binary => 1 );
...
sub export_object_stl {
	...
	my $output_file = $self->_get_export_file('STL') or return;
	Slic3r::Format::STL->write_file( $output_file, $model_object->mesh,
		binary => 1 );
	$self->statusbar->SetStatusText("STL file exported to $output_file");
}
...
sub _get_export_file {
	...
	my $output_file = $main::opt{output};
	{
		...
		my $dlg = Wx::FileDialog->new(
			$self,                        "Save $format file as:",
			dirname($output_file),        basename($output_file),
			&Slic3r::GUI::MODEL_WILDCARD, wxFD_SAVE | wxFD_OVERWRITE_PROMPT
		);
		if ( $dlg->ShowModal != wxID_OK ) {
			$dlg->Destroy;
			return undef;
	...
	my $cb = sub {
		...
		if ($Slic3r::have_threads) {
			Wx::PostEvent(
				$self,
				Wx::PlThreadEvent->new(
					-1, $THUMBNAIL_DONE_EVENT, shared_clone( [$obj_idx] )
				)
			);
			Slic3r::thread_cleanup();
			threads->exit;
...
sub on_extruders_change {
	...
	while ( @$choices < $num_extruders ) {
		...
		my $choice =
		  Wx::BitmapComboBox->new( $self, -1, "", wxDefaultPosition,
			wxDefaultSize, [@presets], wxCB_READONLY );
		push @$choices, $choice;

		# copy icons from first choice
		$choice->SetItemBitmap( $_, $choices->[0]->GetItemBitmap($_) )
		  for 0 .. $#presets;

		# insert new choice into sizer
		$self->{presets_sizer}->Insert( 4 + ( $#$choices - 1 ) * 2, 0, 0 );
		$self->{presets_sizer}->Insert(
			5 + ( $#$choices - 1 ) * 2,
			$choice, 0, wxEXPAND | wxBOTTOM,
			FILAMENT_CHOOSERS_SPACING
		);
		...
		my $i = first {
			$choice->GetString($_) eq
			  ( $Slic3r::GUI::Settings->{presets}{ "filament_" . $#$choices }
				  || '' )
		}
		0 .. $#presets;
...
"Please install the OpenGL modules to use this feature (see build instructions)."
		);
		return;
	}

	my $dlg = Slic3r::GUI::Plater::ObjectCutDialog->new(
		$self,
...
sub object_settings_dialog {
	...
	my $dlg = Slic3r::GUI::Plater::ObjectSettingsDialog->new(
		$self,
		object       => $self->{objects}[$obj_idx],
...
sub object_list_changed {
	...
	$self->{"btn_$_"}->$method
	  for grep $self->{"btn_$_"},
	  qw(reset arrange export_gcode export_stl send_gcode);
...
sub selection_changed {
	...
	$self->{"btn_$_"}->$method
	  for grep $self->{"btn_$_"},
	  qw(remove increase decrease rotate45cw rotate45ccw changescale split cut settings);

	if ( $self->{htoolbar} ) {
		$self->{htoolbar}->EnableTool( $_, $have_sel )
		  for (
			TB_REMOVE, TB_MORE,  TB_FEWER, TB_45CW, TB_45CCW,
			TB_SCALE,  TB_SPLIT, TB_CUT,   TB_SETTINGS
		  );
	}
	...
	if ( $self->{object_info_size} ) {   # have we already loaded the info pane?
		if ($have_sel) {
			my $model_object   = $self->{model}->objects->[$obj_idx];
			my $model_instance = $model_object->instances->[0];
			$self->{object_info_size}->SetLabel(
				sprintf( "%.2f x %.2f x %.2f",
					@{ $model_object->instance_bounding_box(0)->size } )
			);
			$self->{object_info_materials}
			  ->SetLabel( $model_object->materials_count );

			if ( my $stats = $model_object->mesh_stats ) {
				$self->{object_info_volume}->SetLabel(
					sprintf( '%.2f',
						$stats->{volume} *
						  ( $model_instance->scaling_factor**3 ) )
				);
				$self->{object_info_facets}->SetLabel(
					sprintf( '%d (%d shells)',
				...
				{
					$self->{object_info_manifold}->SetLabel(
						sprintf( "Auto-repaired (%d errors)", $errors ) );
				  ...
				  # to admesh, so it generates normals for all facets
					my $message =
					  sprintf
'%d degenerate facets, %d edges fixed, %d facets removed, %d facets added, %d facets reversed, %d backwards edges',
					  @$stats{
						qw(degenerate_facets edges_fixed facets_removed facets_added facets_reversed backwards_edges)
					  };
					$self->{object_info_manifold}->SetToolTipString($message);
					$self->{object_info_manifold_warning_icon}
					  ->SetToolTipString($message);
				}
				else {
			...
			$self->{"object_info_$_"}->SetLabel("")
			  for qw(size volume facets materials manifold);
			$self->{object_info_manifold_warning_icon}->Hide;
...
sub selected_object {
	...
	my $obj_idx =
	  first { $self->{objects}[$_]->selected } 0 .. $#{ $self->{objects} };
...
sub validate_config {
	...
	eval { $self->GetFrame->config->validate; };
...
sub object_menu {
	...
	$frame->_append_menu_item(
		$menu,
		"Increase copies\tCtrl++",
		'Place one more copy of the selected object',
		sub {
			$self->increase;
		},
		undef,
	...
	$frame->_append_menu_item(
		$menu,
		"Decrease copies\tCtrl+-",
		'Remove one copy of the selected object',
		sub {
			$self->decrease;
		},
		undef,
	...
	$frame->_append_menu_item(
		...
		'Change the number of copies of the selected object',
		sub {
	...
	$frame->_append_menu_item(
		$menu,
		"Rotate 45° clockwise",
		'Rotate the selected object by 45° clockwise',
		sub {
			$self->rotate(-45);
		},
		undef,
	...
	$frame->_append_menu_item(
		...
		'Rotate the selected object by 45° counter-clockwise',
		sub {
			$self->rotate(+45);
		},
		undef,
	...
	my $rotateMenuItem = $menu->AppendSubMenu( $rotateMenu, "Rotate",
		'Rotate the selected object by an arbitrary angle' );
	$frame->_set_menu_item_icon( $rotateMenuItem, 'textfield.png' );
	$frame->_append_menu_item(
		$rotateMenu,
		"Around X axis…",
		'Rotate the selected object by an arbitrary angle around X axis',
		sub {
			$self->rotate( undef, X );
		}
	);
	$frame->_append_menu_item(
		$rotateMenu,
		"Around Y axis…",
		'Rotate the selected object by an arbitrary angle around Y axis',
		sub {
			$self->rotate( undef, Y );
		}
	);
	$frame->_append_menu_item(
		$rotateMenu,
		"Around Z axis…",
		'Rotate the selected object by an arbitrary angle around Z axis',
		sub {
			$self->rotate( undef, Z );
		}
	);
	...
	#=for comment
	my $PositionMenu     = Wx::Menu->new;
	my $PositionMenuItem = $menu->AppendSubMenu( $PositionMenu, "Position",
		'Rotate the selected object by an arbitrary angle' );
	$frame->_set_menu_item_icon( $PositionMenuItem, 'textfield.png' );
	$frame->_append_menu_item(
		$PositionMenu,
		"Around X axis…",
		'Rotate the selected object by an arbitrary angle around X axis',
		sub {
			$self->setPosition( undef, X );
		}
	);
	$frame->_append_menu_item(
		$PositionMenu,
	...
	$frame->_append_menu_item(
		$flipMenu,
		"Along X axis…",
		'Mirror the selected object along the X axis',
		sub {
			$self->flip(X);
		}
	);
	$frame->_append_menu_item(
		$flipMenu,
		"Along Y axis…",
		'Mirror the selected object along the Y axis',
		sub {
			$self->flip(Y);
		}
	);
	$frame->_append_menu_item(
		$flipMenu,
		"Along Z axis…",
		'Mirror the selected object along the Z axis',
		sub {
			$self->flip(Z);
		}
	);
	...
	my $scaleMenuItem = $menu->AppendSubMenu( $scaleMenu, "Scale",
		'Scale the selected object along a single axis' );
	$frame->_set_menu_item_icon( $scaleMenuItem, 'arrow_out.png' );
	$frame->_append_menu_item(
		$scaleMenu,
		"Uniformly…",
		'Scale the selected object along the XYZ axes',
		sub {
	...
	$frame->_append_menu_item(
		$scaleMenu,
		"Along X axis…",
		'Scale the selected object along the X axis',
		sub {
			$self->changescale(X);
		}
	);
	$frame->_append_menu_item(
		$scaleMenu,
		"Along Y axis…",
		'Scale the selected object along the Y axis',
		sub {
			$self->changescale(Y);
		}
	);
	$frame->_append_menu_item(
		$scaleMenu,
		"Along Z axis…",
		'Scale the selected object along the Z axis',
		sub {
			$self->changescale(Z);
		}
	);
	...
	$frame->_append_menu_item(
		$menu, "Split",
		'Split the selected object into individual parts',
		sub {
			$self->split_object;
		},
		undef,
	...
	$frame->_append_menu_item(
		...
		'Export this single object as STL file',
		sub {
...
has 'thumbnail' => ( is => 'rw' )
  ;    # ExPolygon::Collection in scaled model units with no transforms
has 'transformed_thumbnail' => ( is => 'rw' );
has 'instance_thumbnails' => ( is => 'ro', default => sub { [] } )
  ; # array of ExPolygon::Collection objects, each one representing the actual placed thumbnail of each instance in pixel units
...
sub make_thumbnail {
	...
	if ( $mesh->facets_count <= 5000 ) {
		...
		$self->thumbnail->append(
			...
			,    # horizontal_projection returns scaled expolygons
		);
